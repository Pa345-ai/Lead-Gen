...
// FINDING #1: Direct OOB Trigger Test
// This test calls normalizePathEncoding() directly with malformed input
// to bypass ada's normalization and trigger the OOB access if it exists
void testFinding1DirectOOB() {
  // Test case 1: pathname ending with "%2"
  kj::String path1 = kj::str("/foo%2");
  auto result1 = normalizePathEncoding(path1);
  
  // Test case 2: just "%2"
  kj::String path2 = kj::str("%2");
  auto result2 = normalizePathEncoding(path2);
  
  // Test case 3: "foo%2"
  kj::String path3 = kj::str("foo%2");
  auto result3 = normalizePathEncoding(path3);
  
  // If ASAN detects OOB, test will crash here
  // If it passes, either ada prevents the issue or findNext() never matches
}
