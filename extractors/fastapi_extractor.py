...
"""
FastAPI AST Extractor – with ownership check parsing.
...
Uses tree-sitter to walk Python source files and extract:
  ...
  - Ownership checks inside function bodies (e.g., if resource.user_id != current_user.id)
"""
...
class FastAPIExtractor:
    ...
    def _try_extract_route_decorator(
        ...
        for ac in auth_contexts:
            if ac.source.startswith("body_ownership_check"):
                # Skip adding permission nodes for inline checks (they are not functions)
                continue
    ...
    # ── NEW: Scan body for explicit ownership checks ──────────────────────────
    def _scan_body_for_ownership_check(self, body_node: TSNode, src: bytes, endpoint_id: str) -> list[AuthContext]:
        """
        Look for patterns like:
            if resource.user_id != current_user.id: raise HTTPException(404)
            if order.user_id != current_user.id:
                raise HTTPException(status_code=404)
        Returns list of AuthContext objects with inferred ownership constraints.
        """
        body_text = _text(body_node, src)
        contexts = []
        # Pattern: if X.user_id != current_user.id or X.owner_id == current_user.id
        # Capture both inequality (access denied) and equality (access granted)
        patterns = [
    ...
    # ── External call extraction (unchanged) ──────────────────────────────────
    ...
    # ── Dependency function extraction (unchanged) ────────────────────────────
    ...
    # ── Post-processing: infer path params (unchanged) ────────────────────────
